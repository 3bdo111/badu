...
export const serverOrderRepository = {
  async createOrder(customer: OrderCustomerInfo, cartItems: OrderItemInput[]): Promise<OrderRecord> {
    ...
    await db.transaction(async (tx) => {
      ...
      for (const item of cartItems) {
        const product = await tx.get<{
          ...
          name_en: string;
          name_ar: string;
        }>("SELECT * FROM products WHERE (id = ? OR slug = ?) AND available = 1", [item.productId, item.productId]);

        if (!product) {
          throw new Error(`Product is no longer available.`);
        }

        const stockRow = await tx.get<{ quantity: number }>(
        ...
        const colorRow = await tx.get<{ name_en: string; name_ar: string }>(
          "SELECT name_en, name_ar FROM product_colors WHERE product_id = ? LIMIT 1",
          [product.id]
        );
        ...
        const stockResult = await tx.run(
          "UPDATE product_stock SET quantity = quantity - ? WHERE product_id = ? AND size = ? AND quantity >= ?",
          [item.quantity, product.id, item.size, item.quantity]
        );
    ...
    const created = await this.getOrderByNumber(orderNumber);
    return created!;
  },

  async getOrders(query?: string, statusFilter?: string): Promise<OrderRecord[]> {
    const db = await getDb();
    ...
    const orders = await db.all<DbOrderRow>(sql, params);
    const allItems = await db.all<DbOrderItemRow>("SELECT * FROM order_items");
  ...
  async getOrderById(id: string): Promise<OrderRecord | undefined> {
    const db = await getDb();
    const ord = await db.get<DbOrderRow>("SELECT * FROM orders WHERE id = ?", [id]);
    if (!ord) return undefined;

    const items = await db.all<DbOrderItemRow>("SELECT * FROM order_items WHERE order_id = ?", [id]);
    return mapOrderRows(ord, items);
  },

  async getOrderByNumber(orderNumber: string): Promise<OrderRecord | undefined> {
    const db = await getDb();
    const ord = await db.get<DbOrderRow>("SELECT * FROM orders WHERE order_number = ?", [orderNumber]);
    if (!ord) return undefined;

    const items = await db.all<DbOrderItemRow>("SELECT * FROM order_items WHERE order_id = ?", [ord.id]);
    return mapOrderRows(ord, items);
  },

  async updateOrderStatus(id: string, newStatus: OrderStatus): Promise<OrderRecord | undefined> {
    const ord = await this.getOrderById(id);
    if (!ord) return undefined;
    ...
    await db.transaction(async (tx) => {
      ...
      if (newStatus === "CANCELLED" && !ord.stockRestored) {
        ...
        await tx.run("UPDATE orders SET stock_restored = 1 WHERE id = ?", [id]);
      }
      ...
      await tx.run(
        `UPDATE orders
         ...
         WHERE id = ?`,
        [newStatus, paymentStatus, now, id]
      );
    });
